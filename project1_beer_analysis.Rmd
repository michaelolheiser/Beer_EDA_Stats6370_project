...
We will be exploring a data set of various beers and breweries provided by Anheuser Busch to assess the relationship between International Bitterness Units (IBU) and Alcohol By Volume (ABV). We will also seek to share other insights such as below

1. Distribution of breweries by state
2. More on Alcohol content and bitterness of beers and its relationship
3. Classification of beers
4. Conclusion with key takeways and oppurtunities
...
## Joining Beer and Breweries dataset
...
## Insights from KNN Classfication of IPA vs Other Ales

1) IBU are clearly higher for IPA than other Ales and span a larger range.
2) IPAs tend to be higher ABV than Other Ales
...
knn_results<-data.frame(
          BeerStyle=c( "American Amber / Red Ale",
                       ...
                       "American IPA", 
                       "American Pale Ale (APA)"),
           Accuracy=c(CM$byClass[1,11], CM$byClass[2,11], CM$byClass[3,11], CM$byClass[4,11], CM$byClass[5,11]),
           Sensitivity=c(CM$byClass[1,1], CM$byClass[2,1], CM$byClass[3,1], CM$byClass[4,1], CM$byClass[5,1]),
           Specificity=c(CM$byClass[1,2], CM$byClass[2,2], CM$byClass[3,2], CM$byClass[4,2], CM$byClass[5,2])
        )
...
## Insights from KNN Classfication of Top 5 Common Beer Styles

1) American Amber/Red and Blonde Ales are difficult to classify due to the overlap with other styles.
2) Pale Ale,IPA, and Imperial IPAs are more distinctly separated by IBU/ABV
...
1) There are no obvious regional patterns in the distribution of breweries
2) IBU are not regularly reported
3) There is a moderate positive linear relationship between IBU and ABV
4) Bi-modal distribution of IBU, could mean possibly different markets
5) Not all styles of beer can be accurately classified by IBU/ABV

Further Research:

1) Distribution/Sales data – where is the beer going?
2) Demographics, economics of states
