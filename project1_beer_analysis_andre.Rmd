---
title: 'Epxloratory Data Analysis of Beers and Breweries'
...
# Plot the comparison between IPA and other ales
...
#KNN comparison between IPA and ather ales
...
for(i in 1:iterations)
{
    ...
    t = table(test$Style,classifications)
    CM = confusionMatrix(t)
  accs$accuracy[i] = CM$overall[1]
  accs$k[i] = i
}
plot(accs$k,accs$accuracy, type = "l", xlab = "k", ylab="Accuracy")
classifications = knn(train[,c(3,4)],test[,c(3,4)],train$Style, prob = TRUE, k = 9)
t = table(test$Style,classifications)
CM = confusionMatrix(t)
CM
```
...
for(j in 1:iterations)
{
  ...
  trainIndices = sample(1:dim(IPAandOther)[1],round(splitPerc * dim(IPAandOther)[1]))
  train = IPAandOther[trainIndices,]
  test = IPAandOther[-trainIndices,]
  
  model = naiveBayes(train[,c(3,4)],as.factor(train$Style))
  table(predict(model,test[,c(3,4)]),as.factor(test$Style))
  CM = confusionMatrix(table(predict(model,test[,c(3,4)]),as.factor(test$Style)))
