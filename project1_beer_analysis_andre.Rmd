...
```{r}
ColoradoData = beer_merged %>% filter(State == "CO" & between(IBU, 65,75))
ColoradoData %>% group_by(City) %>% count(City)
CityPlot = ColoradoData %>% group_by(City) %>% 
  summarize(count = n()) %>% 
  ...
  labs(title="Number of Breweries Per State",
     ...
     theme(axis.text.x = element_text(angle=65, vjust=0.6))
ggplotly(CityPlot)
ColoradoData %>% group_by(City) %>% count(brewery_name)
```
